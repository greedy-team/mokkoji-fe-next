...
`.agents/spec.md`를 작성한 뒤 대화창에 입력:

```
spec-parser 에이전트로 스펙을 실행해줘
```
...
`.agents/references/spec-template.md`를 복사해 `.agents/spec.md`로 저장 후 채웁니다.
...
- CSS: .agents/figma/my-comment/comment-card.txt

## 도메인 컴포넌트 (Block)
### MyCommentItem
- domain: my-comment
- 역할: 내 댓글 목록 아이템 (my-comment에서만 사용)
- CSS: .agents/figma/my-comment/my-comment-item.txt
```
...
3. `.agents/figma/{feature}/{component-name}.txt`로 저장
