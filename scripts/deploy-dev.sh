...
echo "개발 환경 무중단 배포를 시작합니다"

if [ -f "nginx/active/dev-color.txt" ]; then
    CURRENT_COLOR=$(cat nginx/active/dev-color.txt)
else
    CURRENT_COLOR="blue"
    echo "blue" > nginx/active/dev-color.txt
fi
...
docker-compose up -d app-dev-$NEW_COLOR
...
echo "server $NEW_CONTAINER:3000;" > nginx/active/dev-upstream.conf
