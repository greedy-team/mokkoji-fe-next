...
- **역할**: {설명}
- **CSS**: `.agents/figma/{feature}/{component}.txt` (없으면 "없음")
...
- **사용할 Block**:
  - `{BlockName}`: {역할}
  - `{BlockName}`: {역할}
- **CSS**: `.agents/figma/{feature}/{widget}.txt` (없으면 "없음")
...
- **CSS**: `.agents/figma/{feature}/{page}.txt` (없으면 "없음")

---

## 컴포넌트 CSS 목록

> Figma에서 위젯 variant 프레임 선택 → CSS 복사 (All layers) → `.agents/figma/{feature}/{name}.txt`로 저장.
> 위젯 variant 1개당 파일 1개. 경로만 나열합니다.

| 대상 | CSS 파일 경로 |
|------|-------------|
| {ComponentName} - 기본 | `.agents/figma/{feature}/{name}.txt` |
| {WidgetName} - 활성 상태 | `.agents/figma/{feature}/{name}-active.txt` |
