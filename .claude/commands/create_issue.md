...
- **브랜치 베이스**: 항상 `develop`에서 분기한다. (현재 체크아웃된 브랜치나 `main`에서 분기하지 않는다)
...
작업 내용을 한 줄로, 한국어로 간결하게 쓴다.

- 이슈 제목에는 `[#번호]`·`type:` 같은 접두사를 붙이지 않는다. (그건 커밋 메시지 컨벤션이다)
- 60자 이내.

```
# 올바른 예
동아리 목록 캐시로 인한 모집 상태 불일치 수정
Claude Code 커맨드를 레포 템플릿 기준으로 정비

# 잘못된 예
[#724] fix: 캐시 수정     ← 커밋 컨벤션을 이슈에 쓰지 않는다
버그 수정                  ← 무엇인지 알 수 없음
```
