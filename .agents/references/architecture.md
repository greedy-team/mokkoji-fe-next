...
```
Method A: Attach the image directly in Codex
Method B: Specify file path (.agents/figma/{component-name}/{component-name}.png)
```

For CSS: Copy from Figma right panel → "Copy as CSS" → paste in chat
Or provide as css file under `.agents/figma/{component-name}/` folder